"""Stage 3 Slice F — Today pending_regression + sync decision entity."""
...
async def _seed_pending_rec(
    db: AsyncSession, *, user: User, sat
) -> UUID:
    ...
    for day in (date(2026, 8, 3), date(2026, 8, 4)):
        await create_session(
            ...
            body=SessionCreateV1(
                schema_version=1,
                performed_at=datetime(day.year, day.month, day.day, 10, 0, tzinfo=UTC),
                local_date=day,
...
@pytest.mark.asyncio
async def test_today_exposes_pending_regression(db: AsyncSession) -> None:
    user = await _ready(db, "slice-f-today@ex.com")
    ...
    dto = await build_today(db, user=user, local_date=date(2026, 8, 5))
    match = next((s for s in dto.satellites if s.exercise_id == sat.id), None)
    assert match is not None
    assert match.pending_regression is not None
    assert match.pending_regression.id == rec_id
    assert match.pending_regression.from_step == 2
    assert match.pending_regression.to_step == 1
    assert match.current_step_number == 2


@pytest.mark.asyncio
async def test_sync_push_accepts_regression_decision(db: AsyncSession) -> None:
    user = await _ready(db, "slice-f-sync@ex.com")
    sat = await create_satellite(
        db, user=user, body=_copenhagen_body(mutation_id=new_uuid7()), commit=True
    )
    rec_id = await _seed_pending_rec(db, user=user, sat=sat)
